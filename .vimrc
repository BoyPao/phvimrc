...
function! UpdateCscopeDB()
	let curcwd = getcwd()
	exe "cd" . GetCscopeDBPath()
	echohl PreCondit | echo " Updating cscope ..." | echohl None
	"set modifiable
	"delete "cscope.*"
	"set nomodifiable
	:silent !rm -f tags cscope.*
	:silent !find -name "*.c" -o -name "*.cpp" -o -name "*.h" -o -name "*.hpp" -o -name "*.dtsi" -o -name "*.dts" -o -name "Makefile" -o -name "*.mk" > cscope.tags.lst
	:silent !ctags -R --c++-kinds=+p --fields=+iaS --extra=+q < cscope.tags.lst
	silent exe "!cscope -bkq -i cscope.tags.lst"
	silent exe "cs reset"
	call LoadCscope()
	exe "cd" . curcwd
...
function DebugVim(target)
	...
	echo "DebugVim +++"
	echo findfile("plug.vim", $HOME . "/.vim/autoload/**")
	echo "DebugVim ---"
endfunction
...
if !empty(findfile("plug.vim", $HOME . "/.vim/autoload/**"))
	...
	" Regular vim plugins (ctags & cscope should installed in system)
